...
Tyontekija::Tyontekija(string etunimi_, string osoite_, string puhelinnumero_, string sukunimi_, int palkka_, string tunnus_)
...
void Tyontekija::asetaPalkka(int palkka_)
{
	palkka = palkka_;
}
...
void Tyontekija::kysyTiedot()
{
	...
	try {
		...
		cin.ignore(10000, '\n');
		if (palkka == 0) { throw 124; }
	}
	catch (int errornum)
	{
		cout << "Palkka ei voi olla 0 ! error:" <<endl<< errornum;
		kysyTiedot();
	}
	cout << "Anna tunnus! >>";
	getline(cin, tunnus);
